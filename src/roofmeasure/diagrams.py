...
class _Vec:
    ...
    def __init__(self, m: RoofMeasurements, detail: MeasureDetail):
        sk = getattr(detail, "skeleton", None)
        if sk is not None and getattr(sk, "ok", False):
            # geometrically consistent wireframe (preferred)
            self.outline = sk.outline
            self.edges = [{"p0": e.p0, "p1": e.p1, "kind": e.kind,
                           "length_ft": e.length_ft} for e in sk.edges]
            self.internal = [e for e in self.edges
                             if e["kind"] in ("ridge", "hip", "valley")]
            # faces: letters assigned smallest -> largest area
            order = sorted(sk.face_area_sqft, key=lambda i: sk.face_area_sqft[i])
            letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
            self.faces = []
            for rank, i in enumerate(order):
                letter = letters[rank % 26] * (rank // 26 + 1)
                self.faces.append({
                    "letter": letter,
                    "centroid": sk.face_centroids.get(i, (0, 0)),
                    "pitch": sk.face_pitch.get(i, m.predominant_pitch),
                    "area_sqft": sk.face_area_sqft[i],
                    "downslope": sk.face_downslope.get(i, (0.0, 0.0)),
                })
        else:
            # raster-evidence fallback
            self.outline = outline_polygon(detail)
            outline_edges = classify_outline_edges(self.outline, detail)
            self.internal = internal_lines(detail)
            self.edges = outline_edges + self.internal
            ...
            for letter, (cy, cx) in detail.facet_centroids.items():
                ...
                self.faces.append({
                    ...
                    "centroid": (cy, cx),
                    "pitch": f.pitch,
                    "area_sqft": f.surface_area_sqft,
                    "downslope": (-math.cos(az) * -1.0, math.sin(az)),
                })
...
def render_diagram_png(
    ...
    for f in vec.faces:
        if f["area_sqft"] < 20:
            continue
        cy, cx = f["centroid"]
        ax.text(cx, cy, f["letter"], fontsize=8, fontweight="bold", ha="center", va="center",
    ...
    with PdfPages(out_path) as pdf:
        ...
        for f in vec.faces:
            if f["area_sqft"] < 10:
                continue
            cy, cx = f["centroid"]
            ax.text(cx, cy, f"{f['letter']}\n{f['area_sqft']:,.0f}", fontsize=7.5,
